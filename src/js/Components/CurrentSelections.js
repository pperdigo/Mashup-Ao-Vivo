...
function CurrentSelections() {
  ...
  if (selections.length > 0) {
    content = selections.map((item) => {
      return (
        <div key={item.field + "/" + item.field} className="selected-block">
          <div className="current-information">
            ...
            <div className="filters-items">
              ...
              <button
                onClick={() => {
                  clearField(item.field);
                }}
                className="x-clear-btn"
              >
                <img src={closeIcon} alt="" width="10px" />
              </button>
            </div>
          </div>
        </div>
  ...
  return <div className="current-selections custom-font">{content}</div>;
}
